...
#SBATCH -t 8:00:00                            # Time (minutes)
#SBATCH -a 1-n                          # Array range
#SBATCH -o /mnt/.../logs/%x-%A_%a.log
#SBATCH -e /mnt/.../logs/%x-%A_%a.log

INDEX=$SLURM_ARRAY_TASK_ID
# CD to directory
cd /path/to/UoM_ATAC_processor
...
python -X faulthandler ./src/main_ATAC.py -i "${SAMPLE}" --config ./configs/config.yaml --threads "${TOOL_THREADS}" \
